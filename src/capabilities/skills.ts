...
export async function installSkillFromBody(
  ...
  for (const file of input.files ?? []) {
    ...
    // agent's terminal_exec wrapper (see ADR connector-provider-spec-compliance §Skills); land them
...
export async function updateSkill(config: RuntimeConfig, idOrName: string, input: Record<string, unknown>) {
  return mutateState(config.instance, (state) => {
    ...
    // decision, not a content edit. ADR connector-provider-spec-compliance §UI requires this surface.
    if (

...
describe("azure openai routing", () => {
  const AZURE_BASE = "https://lilac-labs-w.openai.azure.com";
  const AZURE_URL =
    "https://lilac-labs-w.openai.azure.com/openai/deployments/gpt-5.4/chat/completions?api-version=2024-12-01-preview";

  test("normalizeProvider carries azure fields for openai and drops them elsewhere", () => {
    const azure = normalizeProvider({
      name: "openai",
      model: "gpt-5.4",
      baseUrl: AZURE_BASE,
    ...
    expect(azure.deployment).toBe("gpt-5.4");
    expect(azure.authScheme).toBe("api-key");

    // Whitespace-only values are treated as absent.
    const blank = normalizeProvider({ name: "openai", model: "m", apiVersion: "   ", deployment: "  " });
    expect(blank.apiVersion).toBeUndefined();
    expect(blank.deployment).toBeUndefined();

    // A bogus authScheme falls back to the bearer default (field omitted).
    const badScheme = normalizeProvider({
      name: "openai",
      model: "m",
      authScheme: "weird" as unknown as "bearer"
    });
    expect(badScheme.authScheme).toBeUndefined();

    // Non-openai providers never carry azure routing fields.
    ...
    expect(local.apiVersion).toBeUndefined();
    expect(local.deployment).toBeUndefined();
    expect(local.authScheme).toBeUndefined();
  });

  test("azureRoutingNeedsBaseUrl flags azure-mode openai configs missing a resource baseUrl", () => {
    // Not openai → never flagged (azure routing is openai-only).
    expect(azureRoutingNeedsBaseUrl("local", "2024-12-01-preview", undefined)).toBe(false);
    // openai but not in azure mode (no/blank apiVersion) → not flagged.
    expect(azureRoutingNeedsBaseUrl("openai", undefined, "https://x.openai.azure.com")).toBe(false);
    expect(azureRoutingNeedsBaseUrl("openai", "  ", "https://x.openai.azure.com")).toBe(false);
    // azure mode but no baseUrl → flagged.
    expect(azureRoutingNeedsBaseUrl("openai", "2024-12-01-preview", undefined)).toBe(true);
    expect(azureRoutingNeedsBaseUrl("openai", "2024-12-01-preview", "   ")).toBe(true);
    // azure mode with the default api.openai.com base (even trailing slash) → flagged.
  ...
  test("azure tool-calling posts to the deployment-scoped url with the api-key header", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "azure-secret";
    let captured: { url: string; init: RequestInit } | undefined;
    globalThis.fetch = ((input: RequestInfo | URL, init: RequestInit = {}) => {
      captured = { url: String(input), init };
      const body = {
        id: "az_1",
        choices: [{ finish_reason: "stop", message: { role: "assistant", content: "hi from azure" } }],
    ...
    try {
      ...
      const result = await generateToolCallingResponse(config(provider), [{ role: "user", content: "hello" }], []);
      expect(result.text).toBe("hi from azure");
      expect(captured?.url).toBe(AZURE_URL);
  ...
  test("azure tool-calling deployment defaults to the model id when omitted", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "azure-secret";
    let capturedUrl = "";
    globalThis.fetch = ((input: RequestInfo | URL) => {
      capturedUrl = String(input);
      const body = { id: "az_2", choices: [{ finish_reason: "stop", message: { role: "assistant", content: "ok" } }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } }));
    }) as unknown as typeof fetch;
    try {
      // deployment omitted → falls back to the model id (also "gpt-5.4").
      const provider = normalizeProvider({
        name: "openai",
        model: "gpt-5.4",
        baseUrl: AZURE_BASE,
      ...
      await generateToolCallingResponse(config(provider), [{ role: "user", content: "hi" }], []);
      expect(capturedUrl).toBe(AZURE_URL);
    } finally {
      globalThis.fetch = originalFetch;
      if (original === undefined) delete process.env.OPENAI_API_KEY;
      else process.env.OPENAI_API_KEY = original;
  ...
  test("azure tool-calling with the bearer scheme sends an Entra Authorization header", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "entra-token";
    let captured: { url: string; init: RequestInit } | undefined;
    globalThis.fetch = ((input: RequestInfo | URL, init: RequestInit = {}) => {
      captured = { url: String(input), init };
      const body = { id: "az_3", choices: [{ finish_reason: "stop", message: { role: "assistant", content: "ok" } }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } }));
    }) as unknown as typeof fetch;
    try {
      const provider = normalizeProvider({
  ...
  test("azure tool-calling streams from the deployment-scoped url", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "azure-secret";
    let capturedUrl = "";
    globalThis.fetch = ((input: RequestInfo | URL) => {
      capturedUrl = String(input);
      const events = [
        { choices: [{ index: 0, delta: { content: "az" } }] },
        { choices: [{ index: 0, delta: {}, finish_reason: "stop" }] }
      ];
      const stream = new ReadableStream<Uint8Array>({
        start(controller) {
  ...
  test("azure structured output posts to the deployment-scoped url with api-key", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "azure-secret";
    let captured: { url: string; init: RequestInit } | undefined;
    globalThis.fetch = ((input: RequestInfo | URL, init: RequestInit = {}) => {
      captured = { url: String(input), init };
      const body = { id: "az_s", choices: [{ message: { role: "assistant", content: '{"value":42}' } }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } }));
    }) as unknown as typeof fetch;
    try {
      const provider = normalizeProvider({
      ...
      const result = await generateStructured(config(provider), {
        ...
        user: "u",
        schemaName: "Thing",
        validator: { parse: (v) => v as { value: number } }
      });
  ...
  test("azure vision posts to the deployment-scoped url and keeps max_completion_tokens", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "azure-secret";
    let captured: { url: string; init: RequestInit } | undefined;
    globalThis.fetch = ((input: RequestInfo | URL, init: RequestInit = {}) => {
      captured = { url: String(input), init };
      const body = { id: "az_v", choices: [{ message: { role: "assistant", content: "a cat" } }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } }));
    }) as unknown as typeof fetch;
    try {
      const provider = normalizeProvider({
  ...
  test("azure summary routes through chat/completions, not /responses", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "azure-secret";
    let capturedUrl = "";
    globalThis.fetch = ((input: RequestInfo | URL) => {
      capturedUrl = String(input);
      const body = { id: "az_sum", choices: [{ message: { role: "assistant", content: "summary" } }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } }));
    }) as unknown as typeof fetch;
    try {
      const provider = normalizeProvider({
      ...
      const result = await generateTaskSummary(config(provider), "do a thing");
      expect(result.text).toBe("summary");
      expect(capturedUrl).toBe(AZURE_URL);
  ...
  test("non-azure openai still posts to the flat chat/completions path with Bearer", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    process.env.OPENAI_API_KEY = "plain-key";
    let captured: { url: string; init: RequestInit } | undefined;
    globalThis.fetch = ((input: RequestInfo | URL, init: RequestInit = {}) => {
      captured = { url: String(input), init };
      const body = { id: "oa_1", choices: [{ finish_reason: "stop", message: { role: "assistant", content: "ok" } }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } }));
    }) as unknown as typeof fetch;
    try {
      const provider = normalizeProvider({ name: "openai", model: "gpt-5.4-mini" });
      await generateToolCallingResponse(config(provider), [{ role: "user", content: "hi" }], []);
  ...
  test("api-key auth scheme without apiVersion falls back to Bearer on the flat path", async () => {
    const original = process.env.OPENAI_API_KEY;
    const originalFetch = globalThis.fetch;
    ...
    globalThis.fetch = ((input: RequestInfo | URL, init: RequestInit = {}) => {
      captured = { url: String(input), init };
      const body = { id: "oa_2", choices: [{ finish_reason: "stop", message: { role: "assistant", content: "ok" } }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } }));
    }) as unknown as typeof fetch;
    try {
      // authScheme api-key but NO apiVersion → not Azure mode. The api-key
      // header is meaningless against standard OpenAI, so it must degrade to
      // Bearer on the flat path rather than emit a guaranteed-401 header.
      const provider = normalizeProvider({ name: "openai", model: "gpt-5.4-mini", authScheme: "api-key" });
      await generateToolCallingResponse(config(provider), [{ role: "user", content: "hi" }], []);
      expect(captured?.url).toBe("https://api.openai.com/v1/chat/completions");
      const headers = captured!.init!.headers as Record<string, string>;
      expect(headers.authorization).toBe("Bearer stray-key");
      expect(headers["api-key"]).toBeUndefined();
    } finally {
      globalThis.fetch = originalFetch;
      if (original === undefined) delete process.env.OPENAI_API_KEY;

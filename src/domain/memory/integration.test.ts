...
describe("phase 5 — auto-recall on task submit", () => {
  test("a follow-up task records that hindsight units were recalled", async () => {
    ...
    await Bun.sleep(150);
    expect(listMemoryUnits(lane, DEFAULT_BANK_ID).length).toBeGreaterThan(0);
    ...
    const trace = readTrace(lane, second.id);

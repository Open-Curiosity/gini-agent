    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          triage: { type: "boolean", description: "For action='add': set `true` to TRIAGE THE WHOLE INBOX — every newly-arrived email no specific watch claims gets a proposed reply or a flag — or `false` to turn that off. OPT-IN: only set when the user explicitly asks to handle all their new mail ('watch my whole inbox', 'triage everything'); NEVER set it when they name a specific sender/thread (that's a targeted watch via `sender`/`threadId`). Can be sent on its own (no `sender`/`query`/`threadId`) to toggle triage without creating a targeted watch." }
        },
        required: ["action"]
      }
    }

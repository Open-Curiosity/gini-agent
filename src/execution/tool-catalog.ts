...
export function buildToolCatalog(state: RuntimeState, agentToolsetFilter?: Set<string>): ToolCatalogTool[] {
  ...
  return allTools().filter((tool) => {
    ...
    // lives on `core` (not a legacy default toolset); the model must be able
    // to answer "what time is it" on any instance regardless of which
    // toolsets are enabled, and it has no side effects to gate.

...
const TOOL_DEFS: Array<ToolFunctionSpec & { toolset: string }> = [
  ...
  },
  {
    ...
    function: {
      name: "edit_soul",
      description: "Propose an edit to the active agent's SOUL.md (per-agent persona). Prefer `action: \"set\"` with the full consolidated SOUL.md body — you can see the current state in the system prompt, so emit the new version with the new rule integrated rather than appending a chunk below. Only call this when the user's CURRENT message asks for a NEW persona / voice / behavior rule for this agent. The proposed body lands as SOUL.md.proposed and does NOT enter the system prompt until the user approves it via `POST /api/identity-files/soul/approve`. After calling, you MAY briefly mention the approval step in your reply but do NOT otherwise narrate the tool call. `action: \"append\"` adds a new section below the existing content (prefer set instead; append is a legacy fallback). `action: \"remove\"` drops the first paragraph containing the `needle` substring from the existing approved body; requires `needle`. Requires an active agent — there is no per-instance SOUL.",
      parameters: {
        type: "object",
        properties: {
          action: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Edit the instance-scoped USER.md (user profile). Prefer `action: \"set\"` with the full consolidated USER.md content — you can see the current state in the system prompt, so emit the new version with the new fact integrated rather than appending a chunk below. Only call this when the user's CURRENT message contains a NEW durable identity fact about themselves (name, role, location, stable preference) that is NOT already in USER.md. Casual chat, follow-up questions, and topics unrelated to the user themselves are NOT identity facts — most turns should produce ZERO calls. Do NOT narrate the call in your reply: just acknowledge the fact briefly (\"Got it, X.\", \"Noted.\"). Auto-approved: writes go straight to USER.md and ride the system prompt on the next turn. USER.md is instance-scoped so the user's identity bridges across agent switches. The injection scan still gates content that trips a threat pattern. `action: \"append\"` adds a new section (legacy fallback; the storage layer de-duplicates lines that already exist); `action: \"remove\"` drops the first paragraph containing the `needle` substring (requires `needle`). Distinct from edit_soul which still requires user approval.",
      parameters: {
        type: "object",
        properties: {
          action: {

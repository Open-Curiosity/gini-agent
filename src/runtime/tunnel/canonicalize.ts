// Shared path canonicalization used by the Next.js middleware (proxy) and the
// BFF guard. Tests pin both invocation sites. See PLAN.md "Path canonicalization".

export interface CanonicalizeResult {
  ok: true;
  /** The canonical pathname. Preserves a single trailing slash if the input
   *  had one; collapses multiple trailing slashes to one. Bare "/" stays as
   *  "/". */
  path: string;
  /** True when the canonical path ends in a single trailing slash. The deny
   *  list treats `/foo` and `/foo/` as equivalent; this flag lets callers
   *  forward the form the client actually sent without recomputing it. */
  hadTrailingSlash: boolean;
}

export interface CanonicalizeError {
  ok: false;
  reason:
    | "malformed-percent-encoding"
    | "decode-not-stable"
    | "traversal"
    | "dot-segment"
    | "duplicate-slash"
    | "residual-percent"
    | "backslash-or-nul"
    | "embedded-delimiter"
    | "too-long";
}

const MAX_DECODE_ROUNDS = 8;
const MAX_LENGTH = 4096;

/**
 * Strict percent-decoder. Stops at the first malformed sequence (`%ZZ`,
 * trailing `%`, `%2`) and returns null. Tolerant pass-through is unsafe
 * here because a downstream router with a strict decoder would diverge
 * from a tolerant one. See PLAN.md step 1.
 */
function strictPercentDecode(input: string): string | null {
  const bytes: number[] = [];
  for (let i = 0; i < input.length; i += 1) {
    const ch = input.charCodeAt(i);
    if (ch !== 0x25) {
      // not '%', emit as UTF-8 code unit pair when above 0x7f
      if (ch < 0x80) {
        bytes.push(ch);
      } else {
        // re-encode as UTF-8
        const enc = new TextEncoder().encode(input[i]!);
        for (const b of enc) bytes.push(b);
      }
      continue;
    }
    if (i + 2 >= input.length) return null;
    const hi = input.charCodeAt(i + 1);
    const lo = input.charCodeAt(i + 2);
    const hiVal = hexVal(hi);
    const loVal = hexVal(lo);
    if (hiVal < 0 || loVal < 0) return null;
    bytes.push((hiVal << 4) | loVal);
    i += 2;
  }
  try {
    return new TextDecoder("utf-8", { fatal: true }).decode(new Uint8Array(bytes));
  } catch {
    return null;
  }
}

function hexVal(charCode: number): number {
  if (charCode >= 0x30 && charCode <= 0x39) return charCode - 0x30;
  if (charCode >= 0x41 && charCode <= 0x46) return charCode - 0x41 + 10;
  if (charCode >= 0x61 && charCode <= 0x66) return charCode - 0x61 + 10;
  return -1;
}

export function canonicalizePath(input: string): CanonicalizeResult | CanonicalizeError {
  if (input.length > MAX_LENGTH) return { ok: false, reason: "too-long" };

  // Step 1: fixed-point percent-decode with a strict decoder.
  let cur = input;
  let stabilized = false;
  for (let round = 0; round < MAX_DECODE_ROUNDS; round += 1) {
    const next = strictPercentDecode(cur);
    if (next === null) return { ok: false, reason: "malformed-percent-encoding" };
    if (next === cur) {
      stabilized = true;
      break;
    }
    cur = next;
    if (cur.length > MAX_LENGTH) return { ok: false, reason: "too-long" };
  }
  if (!stabilized) return { ok: false, reason: "decode-not-stable" };

  // Step 2: trailing-slash bookkeeping. Preserve a single trailing `/` and
  // collapse `///` → `/`. Bare `/` stays.
  let hadTrailingSlash = false;
  let work = cur;
  if (work.length > 1 && work.endsWith("/")) {
    hadTrailingSlash = true;
    // collapse multiple trailing slashes
    while (work.length > 1 && work.endsWith("//")) {
      work = work.slice(0, -1);
    }
  }

  // Step 6 / 7 / 8 / 9 (length re-check)
  if (work.length > MAX_LENGTH) return { ok: false, reason: "too-long" };
  if (work.includes("%")) return { ok: false, reason: "residual-percent" };
  if (work.includes("\\") || work.includes("\0")) return { ok: false, reason: "backslash-or-nul" };
  if (work.includes("?") || work.includes("#")) return { ok: false, reason: "embedded-delimiter" };

  // Step 3 / 4 / 5: walk segments. Reject "..", ".", and any duplicate
  // interior slash. Bare leading "/" is preserved.
  if (!work.startsWith("/")) {
    // Pathname must be absolute; canonical form requires the leading slash.
    return { ok: false, reason: "duplicate-slash" };
  }
  // Bare "/" — accept early; split logic below would mis-flag the trailing
  // empty segment as a duplicate slash.
  if (work === "/") return { ok: true, path: "/", hadTrailingSlash: false };
  const segments = work.split("/");
  // split() on "/foo//bar/" yields ["", "foo", "", "bar", ""]. An empty
  // interior segment indicates a duplicate slash. The leading empty and
  // (when present) trailing empty are expected from the bare-"/" preserve
  // rule above.
  for (let i = 0; i < segments.length; i += 1) {
    const seg = segments[i]!;
    if (seg === "..") return { ok: false, reason: "traversal" };
    if (seg === ".") return { ok: false, reason: "dot-segment" };
    const isLeading = i === 0;
    const isTrailing = i === segments.length - 1 && hadTrailingSlash;
    if (seg === "" && !isLeading && !isTrailing) {
      return { ok: false, reason: "duplicate-slash" };
    }
  }

  return { ok: true, path: work, hadTrailingSlash };
}

/**
 * Strip a trailing slash for deny / rewrite-trigger matching only. Returns
 * the path as-is when it's bare "/" or carries no trailing slash. Used by
 * deny / rewrite callers to treat `/api/runtime/tunnel` and
 * `/api/runtime/tunnel/` as equivalent.
 */
export function noTrailingSlash(path: string): string {
  if (path.length > 1 && path.endsWith("/")) return path.slice(0, -1);
  return path;
}

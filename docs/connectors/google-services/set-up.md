...
- macOS or Linux. Gini installs the `gws` and `gcloud` command-line tools for you — no package manager required (it downloads a checksum-verified release binary, and uses `bun` or Homebrew only when they're already available).

...
2. **A `GINI_TRUSTED_ORIGINS` entry** — comma-separated full origins (scheme + host + port), e.g. `GINI_TRUSTED_ORIGINS=https://gini-server.tail-xyz.ts.net,http://localhost:3000`. Required for tailnet and public-DNS exposures — this is the knob every manual tunnel (Tailscale serve, ngrok, cloudflared) needs; see [Remote Access](remote-access.md) for the per-provider setup. If the var is set but every entry is malformed, the gate fails closed for every web-bound request on a non-loopback, non-relay front until fixed (loopback and relay-subdomain requests keep working) — a typo bricks the exposure loudly rather than silently downgrading.

...
2. **`GINI_TRUSTED_ORIGINS` unset** — local-dev fallback. The guard accepts an `Origin`-present request only when both the request `Host` is loopback (`localhost`, `127.0.0.1`, or `[::1]`) and the `Origin` matches `Host`; an `Origin`-absent request needs a loopback `Host`. Any non-loopback `Host` is normally refused without an explicit allowlist — set the env var or bind the BFF to loopback only. The one exception is the Cloudflare quick tunnel surface (see below), which authorizes itself out-of-band and rides through an internal `x-gini-tunnel-vetted` marker the BFF cannot accept from outside.

Closing the non-loopback fallback path blocks the DNS-rebinding shape where an attacker page sets `Origin` to a hostname they control but rebinds DNS to the BFF's loopback / tailnet IP — the rebound host equals itself, so a Host-comparison alone would pass. The allowlist (or the loopback restriction) takes that codepath off the table. The tunnel surface escapes both controls through a third path: cloudflared mints a fresh `<random>.trycloudflare.com` hostname every restart (so it cannot appear in `GINI_TRUSTED_ORIGINS`) and the `Host` is honestly non-loopback. The Next.js proxy (`web/src/proxy.ts`) stamps the internal header `x-gini-tunnel-vetted: 1` onto every request that already passed the tunnel secret-path or session-cookie gate, and `guardCsrf` accepts it as proof of prior authorization. The proxy deletes any inbound value before re-stamping (and deletes-without-stamping on the localhost branch), so a remote attacker or co-tenant process cannot forge the marker. The bypass is method-tiered: vetted safe-method Origin-less GETs and vetted Origin-present `Origin == Host` requests skip the loopback-Host check; unsafe methods (POST/PUT/PATCH/DELETE) still require `Origin`, vetted or not.

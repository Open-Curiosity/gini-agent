...
| `channels.discord` + `DISCORD_BOT_TOKEN` | `MessagingBridgeRecord` (kind `discord`) with encrypted bot token | See [Discord Messaging Bridge](adr/discord-bridge.md). **The supervisor won't poll the bridge until you add at least one delivery channel via `gini messaging`** — openclaw stores a per-sender allowlist while gini stores per-channel snowflakes, so the migrator cannot derive the channel list from openclaw state. The migration warning calls this out so it's easy to notice. |

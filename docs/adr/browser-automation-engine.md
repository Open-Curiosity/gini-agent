...
- **`spawned`** — the agent's DEFAULT path. The runtime launches its own branded Chrome (`src/tools/chrome-launch.ts`: `--headless=new` + stealth args + clean UA + per-agent `--user-data-dir` + a free-picked `--remote-debugging-port`) and attaches over CDP, then reaps the exact pid on teardown. Unlike the single instance-level shared handle the other two providers use, spawned handles are keyed PER AGENT (`spawnedHandles` map), so each agent owns an independent Chrome process and profile dir and multiple agents browse concurrently. See [Browser Stealth Identity](browser-stealth-identity.md).

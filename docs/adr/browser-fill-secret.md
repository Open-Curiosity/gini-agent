...
- The gateway's `/api/approvals/<id>/connect` handler is now a three-action seam (`connector.request`, `browser.fill_secret`, `messaging.add_bridge`); see [chat-block-protocol.md](chat-block-protocol.md) for the wire contract and [telegram-bridge.md](telegram-bridge.md) for the messaging branch. New fill-style actions added later (e.g. `browser.fill_otp` with a TTL-bound code) follow the same pattern: extend `Approval.action`, add a branch — ideally as a thin delegate to a bounded module under `src/execution/` like `runFillSecretConnect` or `runMessagingBridgeConnect` — share the same body shape and BFF route.

...
```
[ system: instructions + soul + user-profile + skills + inactive-skills + mcp + bound-jobs ]   ← byte-stable, cacheable
...
Moving the per-turn content out of message 0 and into a tail placed *after* the transcript restores a long stable prefix (`system + full prior transcript`) and confines the uncached region to the small ephemeral tail plus the user message — which have to be re-prefilled every turn regardless.
...
- **The tail is never persisted.** `priorChatMessages` reads only durable `chatMessages` rows; the tail is constructed in `runChatTask` and dropped after the turn. No double-injection, no stale-tail replay, and resume replays the saved working-messages snapshot verbatim.

## Consequences

- **Cache behavior:** for a session with no configuration change, message 0 is byte-identical turn-to-turn, so the automatic prefix cache stays warm across the system message and the full append-only transcript. The uncached region is the ephemeral tail plus the user message, which are inherently per-turn.
...
- Two consecutive turns in the same session with no identity/skill/job/connector change produce a byte-identical message-0 `content`, and `messages[0].role === "system"`.
...
- `src/system-prompt.ts` — `buildAgentSystemContext` (byte-stable prefix only) and `renderEphemeralContext` (the identity + recalled-memory tail body, single-sourcing the `Long-term memory…` header).
- `src/execution/chat-task.ts` — `runChatTask` builds the stable prefix, then injects the `renderEphemeralContext` tail as a `role:"user"` message between the prior transcript and the real user message, gated to the non-subagent path.

...
- Per-task agent stamp. Each inbound message stamps the allowlist entry's `agentId` onto the submitted `Task` row (via `submitTask`'s `agentId` option). The chat-task loop reads that per-task override when resolving provider / toolset / memory namespace per ADR agent-memory-isolation.md, instead of consulting the instance-wide active-agent pointer. This makes concurrent inbound dispatches from different allowlist entries safe — each task executes under its own agent regardless of the order in which their asynchronous runLoop microtasks fire.
...
- **Callback approval revoke window.** When an allowlisted user taps an inline-keyboard `[Approve]` button, the handler does a live `readState` re-validation of bridge and allowlist (the handler at `src/integrations/messaging/telegram-handlers.ts:handleCallbackQuery`), then calls `decideApproval`. A revoke or disable that lands in the brief window between that live re-read and the `mutateState` inside `resolveApproval` will not block the approval's side effect; the universal approval state machine only re-checks `approval.status === "pending"`, not bridge or allowlist state. This is **semantically equivalent** to a revoke that arrives a few milliseconds after the approval commits — the user already pressed Approve, which is the explicit authorization the side effect needs. The HTTP approval path (`POST /api/approvals/:id/decide`) does not perform any live channel-state re-validation either, so the Telegram path is already stricter than the canonical HTTP path. Closing this race fully would require either folding messaging-channel-specific validation into the universal `decideApproval` flow (polluting the approval state machine across every future channel) or holding an approval-in-flight lock against bridge mutations (introducing deadlock risk when the approve branch runs a side-effecting tool that re-enters the chat-task loop). The race is accepted: user intent is captured at button-press time.

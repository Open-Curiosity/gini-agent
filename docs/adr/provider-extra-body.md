...
`ProviderConfig` carries an optional `extraBody?: Record<string, unknown>` that the runtime merges into the chat-completions request bodies it builds. The `local`, `openrouter`, `deepseek`, and `azure` providers route every call (tool-calling, structured, vision, and the summary) through chat-completions, so `extraBody` applies to all of them. The `openai` provider uses `/responses` for `generateTaskSummary`, so its `extraBody` applies on the tool-calling, structured, and vision calls but not the summary. A reserved-key denylist guards the merge so user-supplied extras can never override fields the runtime owns. Codex (`/responses`) and echo ignore `extraBody` entirely.

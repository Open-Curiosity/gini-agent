...
- **Concurrent writes:** the write-and-rename pattern (same as `state.ts` for the canonical state file) gives crash-atomicity against torn writes, but NOT against lost updates. The `append` and `remove` edits are read-modify-write (load the current body, splice, write back), and concurrent sibling subagents can now overlap — contiguous `spawn_subagent` batches dispatch under `Promise.all` (see ADR [subagent-delegation.md](./subagent-delegation.md)). `USER.md` is per-instance, so any two overlapping subagents in the instance contend on it; `SOUL.md` is shared by same-parent siblings (they inherit the parent's `agentId`). To prevent a lost append/remove, `edit_soul` / `edit_user_profile` serialize their whole read-modify-write under `withIdentityFileLock` — an in-process per-path async mutex keyed by the absolute file path. A single gateway process owns every task in an instance (subagents run in-process via fire-and-forget `runTask`), so an in-process lock suffices; there is no cross-process writer to coordinate with. The approve path is a single atomic rename (no read-modify-write) and does not need the lock.

...
- **The agent's default browser is spawned per agent.** `src/tools/chrome-launch.ts` (`launchSpawnedChrome`) launches a real branded Chrome process directly with `--headless=new`, the shared `CHROME_LAUNCH_ARGS`, the clean UA, a per-agent `--user-data-dir`, and a free-picked `--remote-debugging-port`, then attaches over CDP (`connectOverCDP`). The runtime owns the OS process and reaps it by its exact pid on teardown. This is the `spawned` `BrowserSessionProvider` and is the path every agent tool call takes when no explicit `state.browser` record is set (see [Browser Automation Engine](browser-automation-engine.md)).
...
Profiles are per agent for the spawned default path. `chromeProfileDirFor(instance, agentId)` resolves to `~/.gini/instances/<instance>/agents/<agentId>/chrome-profile`, so each gini agent drives its own Chrome with its own cookies and logins — the same isolation key memory uses (see [Per-Agent Memory Isolation](agent-memory-isolation.md)). A task with no owning agent (system-driven flows) falls back to the legacy per-instance dir `chromeProfileDirFor(instance)` = `~/.gini/instances/<instance>/chrome-profile`, which is also where the visible "Connect" / managed launch persists, so a sign-in done in the visible window is visible to agentless tool calls. The spawned Chromes coexist: each agent owns its own process on its own free-picked debug port, so two agents browse concurrently without sharing a profile lock. Three failure modes broke "login set once stays logged in" and are addressed here:
...
- **A wedged Chrome that won't close** — a page stuck on a heavy/bot-protected navigation can leave `context.close()` unresolved forever, which used to hang the whole teardown. That matters most on the `browser.connect` visible→headless switch ([Authorization vs. Setup Request](authorization-vs-setup-request.md)): `/complete` tears down the visible window and relaunches headless against the same profile dir, so a hung close blocked the relaunch, hung the request for minutes, and stranded the setup request. Teardown now bounds every `close()`/`disconnect()` (`settledWithin`, `teardownCloseTimeoutMs`); on timeout it reaps the managed Chromium by OS pid — playwright-core's `Browser` exposes no `process()`, so the child is found by scanning for the process whose `--user-data-dir` is the instance profile dir and `SIGKILL`ed — which frees the profile-dir lock so the headless relaunch (and the resumed task) proceed. The CDP branch is never killed (the remote Chrome is the user's own process).
...
- The spawned default launch picks a free debug port at or above `DEFAULT_CDP_PORT_BASE` (well above the conventional `9222`), so it never collides with — or attaches to — a user's personal debugging Chrome on `9222`; on teardown it reaps only its own exact pid (never `killall`).
- Two different agents each get their own profile dir under `agents/<agentId>/chrome-profile` and their own Chrome process; an agentless task uses the per-instance `chrome-profile` dir.
- A login (persistent cookie + localStorage) set in an agent's browser is still present after a gateway restart, and after that agent's Chrome is killed and relaunched.

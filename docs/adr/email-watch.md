...
- Startup seeding BASELINES, it does not enumerate. The `after:` watermark only ever moves FORWARD (toward newer mail), so an older un-listed tail can never be reached by advancing it — a seeding path that depends on enumerating the whole backlog would be wrong on a large inbox (gws lists only the newest page-cap worth). Instead, on a watcher with no `lastSeenInternalDate` the first tick takes the NEWEST listed id, fetches only its `internalDate`, sets the cursor there, `markSeen`s just that one boundary id, and wakes NO turn (or, if the inbox had no matches, baselines the cursor at `now`). Pre-existing mail older than the baseline is excluded by `after:` forever — which is the correct behavior (never draft a backlog), regardless of inbox size, with no pagination dependence on first run.
- Steady-state draining of a fully-enumerated (non-truncated) window is OLDEST-FIRST. Each tick enumerates the window, drains it oldest-first, caps the TURNS woken at `MAX_MESSAGES_PER_TICK = 25`, and advances the cursor ONCE at the end to the LAST CONSUMED item's `internalDate` — forward progress that drains a >cap backlog over successive ticks without ever stepping past an un-consumed match. Crash safety is the `email_seen` store, not the cursor: `markSeen` is committed per item, so a crash mid-batch re-lists the window and dedup skips whatever was already handled. When the turn cap is hit with items remaining, the worker logs a warning and the remainder drains on later ticks.
- A TRUNCATED steady-state window (more genuinely-new matches than the page cap of `WINDOW_PAGE_LIMIT * WINDOW_PAGE_SIZE` since the cursor — only reachable after downtime + high volume) is a **bounded skip-with-notice, not an eventual drain**. Because the older tail isn't listed and the forward-only watermark can never reach it, the worker does NOT silently advance past the tail and does NOT storm a draft per message: it jumps the cursor to the NEWEST listed match, `markSeen`s that boundary id, logs the truncation, and wakes exactly ONE notice turn telling the user a large backlog accumulated and that it isn't drafting replies to all of them (it can triage on request). Zero overflow drafts, no infinite re-loop.

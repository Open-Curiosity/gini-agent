...
- Creating a watcher provisions a backing interval-driven job (60s) whose `preRunHook` is `{ handlerId: "skill-script", config: { skill: "gmail-watch", script: "detect", watcherId, query, account? } }`, bound to the watcher's dedicated chat session. The `skill-script` handler runs the bundled `gmail-watch/detect` script HEADLESS and maps its stdout: 0 matches → `shortCircuit` (no turn); 1+ → `context` with each match as a RAW untrusted item the runner fences.
- A gws/transport failure inside the script is reported as a `shortCircuit` (with a status `note`), never a non-zero exit, so the backing JOB stays active and retries next tick — a hook `error` would flip `job.status="failed"`. The handler's `error` kind is reserved for broken config (missing/unknown skill|script, malformed output). Signed-out is the same: a `shortCircuit` with a `needs_auth` note.
- The detection cursor + a small boundary dedup set live on `JobRecord.hookState` (job-owned, persisted at the at-least-once commit boundary above). The watcher record is the durable CONFIG identity; the job holds the detection state. (`config.watcherId` is carried so an orphan backing job can be adopted rather than duplicated.)
- Migration backfill (`backfillEmailWatcherJobs`, run at startup) provisions a backing job for any enabled watcher lacking a resolvable `jobId` (legacy watchers, or a watcher whose job was removed out-of-band). Idempotent: it adopts an existing job (matched by `preRunHook.config.skill === "gmail-watch"` + `watcherId`) and does nothing otherwise, so it's safe on every boot. A migrated watcher re-seeds on its first fire (the cursor lived on the now-absent job state).

...
- The scheduler runs the hook at the same seam in both `runDueJobs` (the scheduled path) and `runJobNow` (manual/replay), between the run claim and `dispatchPromptRun`:
  - **shortCircuit** → `finalizeShortCircuit` finalizes the run with NO model turn. It reuses the normal terminal-task finalize path (`finalizeJobRunFromTask`) via a synthetic terminal `Task` so the run still gets one-shot auto-pause, chat sync, and bridge mirror, with the `[SILENT]`/empty-summary suppression delivering nothing. The synthetic task has no real `taskId` (none was spawned), so it binds to the run via the finalizer's "most-recent running run for this job" match; the finalizer's `status === "running"` guard makes a finalize that races a cancel a no-op (no double finalize).

...
- **Claude in Amazon Bedrock** ("Bedrock Mantle") — `baseUrl` set to `https://bedrock-mantle.{region}.api.aws/anthropic` (the user includes the `/anthropic` path prefix), `apiKeyEnv` pointing at an env var holding a Bedrock API key — either a short-term bearer minted by `aws-bedrock-token-generator` or a long-term IAM service-specific credential (see Operational Notes for the lifetime trade-offs).

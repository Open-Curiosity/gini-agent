...
- **Admin routes** — `GET /api/pairing/requests`, `POST /api/pairing/requests/:id/{approve,reject}`, and `POST /api/pairing` (legacy code create) are admin actions. On the native surface they are `isLoopbackHost`-gated, but a **paired** session (loopback OR relay) reaches them through the BFF (`/api/runtime/pairing/*`) — once paired, a relay session is a full mirror of loopback and can approve/add devices exactly like `127.0.0.1`. The only relay-specific gate is the initial pairing handshake. See [ADR](adr/device-pairing-auth.md) ("Relay sessions mirror loopback").

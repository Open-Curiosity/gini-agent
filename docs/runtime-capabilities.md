...
| Self-config from chat | Gini inspects and reconfigures its own runtime (provider/model, active agent, skills, MCP, connectors) through the `self_discover` / `self_invoke` meta-tools over a `SelfOperation` registry — read ops run inline, config changes route through the approval seam as `self.config`. See [self-config registry ADR](adr/self-config-registry.md) |

...
| Jobs | `gini jobs list/add/run/pause/resume/remove/runs/replay`, prompt jobs, and script jobs. A prompt job's final output posts into its chat channel (when it has one) and is delivered to any messaging bridges named on the job's `deliveryTargets` — jobs without a chat session deliver the task summary (settable via the `create_job`/`update_job` tools, validated against configured Telegram/Discord bridges; see [job channels ADR](adr/agent-chat-threads-and-channels.md)) |

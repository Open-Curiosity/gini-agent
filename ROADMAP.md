...
- ✅ **Cloudflare quick-tunnel with secret-path auth.** Operator opts in to a Cloudflare quick tunnel that exposes the webapp at a random `*.trycloudflare.com` host. A per-instance secret in the URL bootstraps an HttpOnly session cookie; the gateway's CSRF guard accepts the tunnel surface via a proxy-stamped marker that only the same-origin verified handler sets. Optional iCloud Notes mirror writes the QR + URL to a folder so the operator's phone picks them up without manual transfer. Credential-minting routes (pairing) are blocked through the tunnel.

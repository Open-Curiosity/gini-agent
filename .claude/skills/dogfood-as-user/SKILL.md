...
4. **Send the bare request the way the user does** — type it into the web chat UI in a browser (`agent-browser`, so `clientSurface` is `web`), or mobile for a mobile change. One message, no scaffolding. The `gini chat send <session> "<prompt>"` CLI is a fallback for headless contexts only: it posts to the same `/api/chat/<id>/messages` endpoint but skips the surface the user actually touches, so it can't catch a client-side rendering regression.
5. **Observe what the user sees first** — watch the response render in the web UI (`screenshot` to eyeball it). The task's `recentToolCalls` and `~/.gini/instances/<instance>/logs/runtime.jsonl` are a supplement for confirming which tool fired, not the verdict — client-side rendering (chips, narration folding, cards) never shows up there. Judge whether the agent reaches the intended behavior / selects the right tool / emits the right structured affordance **unprompted**.

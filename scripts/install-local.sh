...
log() {
  printf '[gini-install-local] %s\n' "$*"
}

err() {
  printf '[gini-install-local] error: %s\n' "$*" >&2
}

verify_local_repo() {
  if [ ! -e "$LOCAL_REPO/.git" ] || [ ! -f "$LOCAL_REPO/package.json" ]; then
    err "$LOCAL_REPO does not look like a gini-agent checkout (missing .git or package.json)."
    exit 1
  fi
  if ! grep -q '"name": "gini-agent"' "$LOCAL_REPO/package.json"; then
    err "$LOCAL_REPO/package.json does not declare name \"gini-agent\"."
    err "Run this script from inside a gini-agent checkout."
    exit 1
  fi
}
...
fetch_runtime() {
  ...
  if [ -d "$RUNTIME_DIR/.git" ]; then
    ...
    existing_normalized="${existing_origin%/}"
    expected_normalized="${LOCAL_REPO%/}"
    if [ -z "$existing_origin" ] || [ "$existing_normalized" != "$expected_normalized" ]; then
      err "$RUNTIME_DIR is already installed from a different source ($existing_origin). Refusing to overwrite. Run 'gini uninstall --purge' first, or move that directory aside."
      exit 1
    fi
    log "refreshing existing local-test runtime at $RUNTIME_DIR"
    ...
    log "cloning local repo $LOCAL_REPO into $RUNTIME_DIR (--local hardlinks)"
    git clone --local "$LOCAL_REPO" "$RUNTIME_DIR"
  fi
}
...
gini-agent installed from local repo:

  source repo:    $LOCAL_REPO
...
To re-sync after committing changes in $LOCAL_REPO, run:
  gini update
...
main() {
  ...
  log "installing from local repo: $LOCAL_REPO"
